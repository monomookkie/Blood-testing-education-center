...
function MatBadge({ type }) {
  ...
  return <Badge variant={m.color} className="font-mono text-[10px]">{m.label}</Badge>;
}

const EMPTY_COURSE = { title: '', category: '', description: '', status: 'DRAFT', duration: '', passScore: 80, tags: [], materials: [] };
...
export default function CourseManagement({ showToast }) {
  ...
  const filtered = filter === 'all' ? courses : courses.filter(c => c.status === filter.toUpperCase());

  const openNew = () => { setForm(EMPTY_COURSE); setMatForm(EMPTY_MAT); setEditId(null); setShowModal(true); };
  const openEdit = (c) => { setForm({ ...c, tags: c.tags || [], materials: c.materials || [] }); setMatForm(EMPTY_MAT); setEditId(c.id); setShowModal(true); };

  const addMaterial = () => {
    if (!matForm.title.trim()) return;
    setForm(f => ({ ...f, materials: [...f.materials, { ...matForm, id: Date.now().toString() }] }));
  ...
  const handleSave = async () => {
    if (!form.title.trim() || !form.category.trim() || !form.duration) { showToast('Fill required fields', 'error'); return; }
    setLoading(true);
    try {
      if (editId) {
        const updated = await api.updateCourse(editId, form);
        setCourses(cs => cs.map(c => c.id === editId ? { ...updated, tags: updated.tags || [] } : c));
  ...
  return (
    <div className="p-7 page-enter">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-semibold text-navy-900">Course Management</h2>
      ...
      <div className="space-y-3">
        {filtered.map(c => (
          <div key={c.id} className="bg-white rounded-2xl border border-slate-100 p-5 flex items-start gap-4 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2.5 mb-1.5 flex-wrap">
                <span className="font-medium text-navy-900 text-sm">{c.title}</span>
                <Badge variant={c.status === 'PUBLISHED' ? 'green' : 'gray'}>{c.status}</Badge>
                {(c.tags || []).map(t => <Badge key={t} variant="blue" className="text-[10px]">{t}</Badge>)}
              </div>
              <p className="text-xs text-slate-400 mb-2 line-clamp-2">{c.description}</p>
            ...
            <div className="flex gap-2 flex-shrink-0">
              <button onClick={() => openEdit(c)} className="p-2 rounded-lg border border-slate-200 hover:bg-slate-50 text-slate-500 transition-colors"><Icon name="edit" size={14} /></button>
              <button onClick={() => setConfirmDel(c.id)} className="p-2 rounded-lg border border-red-100 hover:bg-red-50 text-red-400 transition-colors"><Icon name="trash" size={14} /></button>
            </div>
          </div>
        ))}
        {filtered.length === 0 && <div className="text-center py-12 text-slate-400 text-sm">No courses found.</div>}
      </div>

      {/* Modal */}
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editId ? 'Edit Course' : 'New Course'} size="700px">
        <div className="space-y-4">
          ...
          {/* Materials */}
          <div>
            <label className="block text-xs font-medium text-slate-500 mb-2">Materials</label>
            <div className="space-y-2 mb-3">
              {form.materials.map((m, i) => (
                <div key={i} className="flex items-center gap-2 p-2.5 bg-slate-50 rounded-xl">
                  ...
                  <button onClick={() => removeMaterial(i)} className="text-slate-400 hover:text-red-500"><Icon name="x" size={13}/></button>
                </div>
              ))}
            </div>
            <div className="flex gap-2">
              <select value={matForm.type} onChange={e => setMatForm(m => ({ ...m, type: e.target.value }))} className="px-2 py-2 rounded-lg border border-slate-200 text-xs bg-slate-50">
                {MATERIAL_TYPES.map(t => <option key={t} value={t}>{t.toUpperCase()}</option>)}
              </select>
              <input value={matForm.title} onChange={e => setMatForm(m => ({ ...m, title: e.target.value }))} placeholder="Title" className="flex-1 px-3 py-2 rounded-lg border border-slate-200 text-xs bg-slate-50" />
              <input value={matForm.url} onChange={e => setMatForm(m => ({ ...m, url: e.target.value }))} placeholder="URL (optional)" className="flex-1 px-3 py-2 rounded-lg border border-slate-200 text-xs bg-slate-50" />
              <button onClick={addMaterial} className="px-3 py-2 bg-brand-500 text-white rounded-lg text-xs hover:bg-brand-600"><Icon name="plus" size={13}/></button>
            </div>
          </div>
      ...
      <ConfirmDialog open={!!confirmDel} title="Delete Course" message="This will permanently delete the course and all its enrollments." onConfirm={handleDelete} onCancel={() => setConfirmDel(null)} />
    </div>
  );
}
